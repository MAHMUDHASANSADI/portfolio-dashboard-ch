<script type="text/javascript">
		...
		var table = $('.datatable-serverside').DataTable({
            ...
            initComplete: function (settings, json) {
                ...
                table.columns().every(function () {
                    ...
                    var input = $('<input type="text" style="width: '+(count == 1 ? '85px' : '100%')+' !important;" class="form-control text-center" placeholder="'+header+'" title="'+header+'"/><span style="display: none">'+header+'</span>')
    ...
    function updateColumnVisibility(ajax_call = true) {
        ...
        if(ajax_call){
            $.ajax({
                url: "{{ url('update-user-column-visibilities') }}",
                type: 'POST',
